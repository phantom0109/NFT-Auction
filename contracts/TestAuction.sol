...
contract TestAuction{
	...
	*  Events
	*/
    event HighestBidIncreased(address bidder, uint256 amount, uint256 endTime, uint256 claimTime);
	event BidWithdrawn(address bidder);
    event AuctionEnded(address winner, uint256 amount, uint256 TestPart, uint256 royaltyAmount);
    event NFTClaimed();
    event BidClaimed();
	...
	constructor(address _Test,
        ...
        // NFT informations
		nftAddr = TestNFT(_nftAddr);
		tokenId = Utility.random(nftAddr.totalSupply());
	...
	*/
	...
	function bid() external payable {
		...
		require(msg.value > 0.01*1 ether, "Too small, it should be at least 0.01");
	...
	function close() external {
        ...
        if (royalty) {
            (royaltyBeneficiary, royaltyAmount) = IERC2981(address(nftAddr)).royaltyInfo(tokenId, highestBid);
        }
		...
		if (highestBid > 0  && bids[highestBidder] > 0) {
			...
			// Transfer auction fee to Test
			uint256 TestPart = 5*(highestBid/ 200);
			payable(Test).transfer(TestPart);
			...
			uint256 amount = highestBid - TestPart - royaltyAmount;
			payable(seller).transfer(amount);
    		emit AuctionEnded(highestBidder, amount, TestPart, royaltyAmount);
		}
	}
    ...
    function claimNFT() external {
        ...
        require(nftAddr.getApproved(tokenId) == address(this), "Contract is not operator");
    ...
    function claimHighestBid() external {
        require(msg.sender == highestBidder, "Only highest bidder can claim funds back");
        require(block.timestamp >= claimTime, "Claim time not started yet");
